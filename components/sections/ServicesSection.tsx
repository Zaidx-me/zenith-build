...
export default function ServicesSection() {
  return (
    <section id="services" className="px-6 md:px-10 py-14 md:py-32">
      <FadeIn y={30} className="text-center mb-16 md:mb-20">
        <span className="section-label">WHAT WE DO</span>
        <h2 className="section-heading mt-4 mb-16 md:mb-20">Services</h2>
      </FadeIn>
        ...
        {services.map((s, i) => {
          ...
          return (
            <FadeIn key={s.title} delay={i * 0.08} y={20}>
              <TiltedCard containerHeight="auto" rotateAmplitude={12} scaleOnHover={1.03}>
                <div className="bg-white/[0.04] border border-white/[0.08] rounded-[32px] p-8 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-5 border border-white/[0.08] bg-white/[0.04]">
                    <Icon className="w-5 h-5 text-[#D7E2EA]" />
                  </div>
                  <h3 className="text-lg font-semibold text-[#D7E2EA] mb-2">
                    {s.title}
                  </h3>
