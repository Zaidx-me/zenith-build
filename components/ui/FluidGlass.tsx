...
interface FluidGlassProps {
  ...
  barProps?: Record<string, unknown> & { navItems?: NavItem[] };
...
function Bar({ modeProps = {}, ...p }: { modeProps?: Record<string, unknown>; [key: string]: unknown }) {
  const defaultMat = {
    transmission: 1,
    roughness: 0,
    thickness: 10,
...
function Images() {
  ...
  useFrame(() => {
    if (!group.current) return;
    const children = group.current.children as unknown as { material: { zoom: number } }[];
  ...
  return (
    <group ref={group}>
      {/* @ts-expect-error drei v10 Image scale accepts 3-element array at runtime */}
